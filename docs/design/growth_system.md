...
>
> ⚠️ 다만 **수치는 코드가 진실이다**(2026-08-14 확인). 문서 작성 이후 XP 곡선이 두 차례 상향됐고
> (`xp_to_next` = 8 + 레벨×3/4), 런 길이도 7스테이지 → **15스테이지**로 늘었다. 아래 "7스테이지 ×
> 평균 1.5레벨업" 같은 페이싱 계산과 체크리스트는 데모 시점 기준이므로 그대로 읽지 말 것.
> 실제 티어 정의는 `scripts/SkillTreeData.gd::LINES`(8라인 × 3티어)가 단일 진실.
...
```
T1 사격 강화         (+1 dmg)
T2 속사              (연사 +40%, 사격 중 감속 완화)
T3 관통              (적 관통, 관통마다 피해 절반)
