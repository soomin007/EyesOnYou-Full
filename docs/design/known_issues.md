...
- **구조 발화 비트(각본 대사)에는 발화 창(전투 유예)을 함께 설계할 것 — 대사와 전투 개시가
  같은 프레임이면 못 읽는다.** 2026-08-17 P2 "어두워지고 뿅뿅대서 설명을 읽을 수 없다"로 온보딩
  유예를 만들고도, P1 인트로·P3 개시·회선 잠금 비트에는 안 깔아 2026-08-23 "대사가 전투랑 겹쳐
  읽을 수가 없다"로 재발(동형 미스윕). → 각본 발화를 넣는 커밋은 그 발화가 흐르는 동안의 위협원
  (스폰·사격·해저드 개시)을 열거해 유예나 조용한 창을 같은 커밋에서 배선한다. 도구 3종:
  ⓐ 개시 유예(스폰 지연 · intro_hold · `_state_t` 음수 시드 — P2 스윕/포탑·SENTINEL·FalseVeil)
  ⓑ 조용한 창(`_combat_near_player` 폴링 대기 · 발화 유실 방지 상한 필수 — 회선 잠금 비트)
  ⓒ 전투 정지 비트(페이즈 전환 despawn — P2 진입). 예외: 조우 직전 위협 콜아웃(드론 밴드 등)은
  눈앞의 적이 지시 대상이라 겹침이 정상.
