...
- **레이 질의는 원웨이 발판을 무시한다 · 거치대 위 저격수가 자기 발판에 시야를 막힌다.**
  증상: 펌프장 1500 거치대 저격이 지상 450px 앞 플레이어를 영영 못 봄(스모크 실측). 24px 발판 위
  총구(-20)에서 아래로 쏘는 레이는 가장자리에서 발판 몸통에 걸려, 기하상 거리 ~800 이하 전부 사각.
  "하향 사선이 지상 중앙에서 겹친다"는 배치 주석이 실제론 성립한 적이 없었다.
  규칙: 시야·사격 레이는 자기 발밑 바디를 제외(`_ray_exclude`, 발밑 짧은 레이로 찾음 · 정지 상태의
  move_and_slide는 충돌을 남기지 않아 get_slide_collision은 못 쓴다)하고, "바로 밑 사각"은 우연한
  기하 대신 명시 규칙(`SNIPER_BLIND_DOWN_DEG` 50°)으로 둔다. 새 원거리 적을 만들 때 같은 함정 확인.
