...
> 맵의 세계 형태(4 템플릿) + 보스/이스터에그/도전 방/웨이브 메커닉 정의.
> Stage.gd + MapData.gd가 이 문서를 따라 동작한다.
>
> ⚠️ 아래 좌표·맵별 명세는 **데모 11맵 시점**에 쓴 것이다(2026-08-14 확인). 현행 맵은 36종이고
> 확장 맵·기믹 맵·처리별 탈출 4종의 좌표는 `scripts/MapData.gd::get_layout`이 단일 진실이다.
> 템플릿·메커닉 정의(이 문서의 본론)는 그대로 유효하며, 새 맵도 이 틀을 따른다.
