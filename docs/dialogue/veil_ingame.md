...
- KO(기본):
  > 레버에서 경험치가 튀어나왔습니다. 챙기십시오.
- KO(warm):
  > 레버에서 경험치가 나왔어요. 놓치지 말고 챙기세요.
- EN:
...
| 앵커 | KO | EN |
|---|---|---|
| 진입 안내(`_show_challenge_briefing`) | 이 안은 통신이 끊깁니다. 발판을 밟으면 시간을 잽니다.⏎맞으면 남은 시간이 깎입니다. | |
| 시간 초과(통신 복구) | 시간 초과입니다. 통신 복구, 조명 켭니다. 출구까지 그대로 가십시오. | |
| 시간 초과 · warm | 시간이 다 됐어요. 통신 돌아왔고 불도 켰으니, 출구까지 그대로 가세요. | |
...
| `_ambience_server_hall` | 서버실 · 코어 접근 | |
