...
- 맥락: 처리별 탈출 4종에서 x 좌표 도달 시 1회 자막.
- 코드: `MapData.gd` `_escape_extract()` `_escape_destroy()` `_escape_conceal()` `_escape_leave()` 각 `route_lines`

| 맵 · 화자 | KO | EN |
|---|---|---|
| 봉쇄 게이트 · veil | 경보가 우릴 앞질러 가요. 다음 구간은 미리 쏘면서 진입하죠. | |
| 붕괴 회랑 · rival | 타는 냄새가 여기까지 옵니다. 제가... 타는 냄새가. | |
| 붕괴 회랑 · rival | 요원. 요원, 요원, 요ㅇ | |
| 붕괴 회랑 · veil | ...신호가 끊겼어요. 저것도, 시설도. 앞만 봐요. | |
