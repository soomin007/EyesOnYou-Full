...
## 4.5 최대 체력 잠금 (막4·막5 문턱, 런당 각 1회 · 2026-08-23 통일 개정)
...
## 8. 14-1 · P3 눈(거짓 VEIL) 3장 구조 (B안 · 2026-09-04)

> '가짜' 어휘 전면 폐지(가짜 눈·가짜 탄·가짜 병사 · 2026-09-03 사용자 확정). 라이벌의 정체성 "렌더를 쥔 자"는
> 적 복제가 아니라 **무대를 다시 그리는 것**으로 표현한다. 1장 겨눔(창 학습) → 2장 추격(도망가며 벽·발판을
> 그린다) → 3장 대면(그릴 방이 없다). 억압(1.4s 컷) 중인 VEIL 줄은 앞부분만 들릴 수 있어 핵심어를 앞에 둔다.
...
- EN:
  > That signal just now. It wasn't mine.

#### 창 안내(그림 상태 · 모습을 드러낼 때만 박힌다)
- 화자: VEIL
- 맥락: P3 오프닝 컷씬 3번째 줄. 화면 = 스캔라인 짜임의 눈 하나(그림 상태). 그려진 벽·발판은 아직
  없으므로 창 문법만 말한다(무맥락 검수 PASS 2026-09-04)
- 코드: `Stage.gd` `_p3_opening_cutscene`
- KO:
  > 저 눈, 지금은 그림입니다. 모습을 드러내는 동안에만 탄이 박힙니다.
- EN:
  > That eye is only a picture right now. Bullets land only while it shows itself.

#### 창 안내 재고지(재시작 · 체크포인트 진입)
- 화자: VEIL
- 맥락: 사망 재시작·체크포인트 진입 1.4s 뒤 전투 중 자막(억압 컷). 컷씬 대신 1줄
- 코드: `Stage.gd` `_p3_restart_line`
- KO:
  > 눈이 드러날 때만 탄이 박힙니다.
- EN:
  > Bullets land only while the eye shows itself.

#### 2장 전환(추격 · 방을 다시 그린다)
- 화자: ?
- 맥락: 1장 몫(HP 30%) 소진 순간. 눈이 먼 자리로 워프(이전 자리 파문) · 요원과 자리 사이 예고 밴드 1.0s →
  그려진 벽 2 + 거짓 발판 2가 나타난다. "벽도, 발판도"는 이 직후 실제로 그려지는 것
- 코드: `Stage.gd` `_on_p3_chapter_changed` (ch 2)
- KO:
  > 잘 보시네요. 그럼 방을 다시 그리죠. 벽도, 발판도.
- EN:
  > Sharp eyes. Then I'll redraw the room. The walls, the floors.

#### 3장 전환(대면 · 그릴 방이 없다)
- 화자: ?
- 맥락: 2장 몫(HP 40%) 소진 순간. 그려진 벽·발판이 띠로 흩어지며 사라지고, 화면 전체 옅은 스캔라인 +
  발판 윤곽 와이어 · 눈이 요원 가까이로 워프해 상시 실체(링 맥동) · 낙하 잔해 시작
- 코드: `Stage.gd` `_on_p3_chapter_changed` (ch 3)
- KO:
  > ...더 그릴 게 없군요. 그럼 이 방 그대로 끝내죠.
- EN:
  > ...Nothing left to draw. Then we finish it in this room, as it is.

#### 지각 보조 · 거짓 발판 표시(1회)
- 화자: VEIL
- 맥락: 신뢰 warm/thaw 밴드 · 요원 320px 안의 거짓 발판에 내 VEIL이 시안 취소선을 처음 그을 때 1회만.
  이후 표시는 취소선(시각)만. cold는 표시 없음(tell = 보라 짜임·슬립은 전원 상시)
- 코드: `Stage.gd` `_veil_assist_tick`
- KO:
  > 저 발판, 눈이 그린 겁니다. 표시해 둡니다.
- EN:
  > That floor was drawn by the eye. Marking it.

#### 캠핑 경고(1회)
- 화자: VEIL
- 맥락: 한자리(300px 창) 2.5s 체류 → 서 있는 발판 윗면을 따라 보라색 선이 차오르는 순간. 1.2s 뒤 그
  발판에 그려진 가시가 솟는다(3.2s 뒤 소멸). 억압 컷이라 행동 지시를 앞에(검수 비고 반영)
- 코드: `Stage.gd` `_tick_p3_camp`
- KO:
  > 자리를 옮기십시오. 발판이 보라색으로 물들면 가시가 솟습니다.
- EN:
  > Move. Purple on the ledge means spikes are coming.
