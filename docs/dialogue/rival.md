...
#### 판별 tell 안내
- 화자: VEIL
- 맥락: P3 오프닝 컷씬 3번째 줄. A안(2026-08-30): 눈이 둘 뜨고 요원을 똑바로 보는 쪽이 진짜.
  재시도 진입 시엔 짧은 자막(`_p3_tell_line` "요원을 똑바로 보는 눈이 진짜입니다.")으로 1.4s 뒤 재고지
- 코드: `Stage.gd` `_p3_opening_cutscene` · `_p3_tell_line`
- KO:
  > 눈이 둘 뜹니다. 요원을 똑바로 보는 쪽이 진짜입니다.
- EN:

#### 무표시 위협 경고
- 화자: VEIL
- 맥락: P3 오프닝 컷씬 4번째 줄. 가짜 눈은 딴 데 보고 탄이 통과한다. 재시도 시 짧은 자막
  (`_p3_unmarked_line` "딴 데 보는 눈은 그림입니다. 탄이 그냥 지나갑니다.")으로 4.8s 뒤
- 코드: `Stage.gd` `_p3_opening_cutscene` · `_p3_unmarked_line`
- KO:
  > 딴 데 보는 쪽은 그림입니다. 탄이 그냥 지나갑니다.
- EN:
...
#### 변주 중반 전이(HP 66%)
- 화자: ?
- 맥락: 변주 중반 · A안(2026-08-30)에선 진짜·가짜 눈의 짝 간격이 한 칸 벌어지는 순간
- 코드: `Stage.gd` `_on_p3_stage_shifted` (stage 1)
- KO:
  > 잘 보시네요. 그럼 둘을 더 벌려 놓죠.
- EN:
