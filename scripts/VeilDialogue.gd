...
# 일반 모드 8스테이지 비트(A1, 3막): 0 도입 / 1 전방안내 / 2 외곽 마무리(내부 진입 직전) /
# 3 흠칫(전조) / 4 봉인구역 / 5 서버 근접(막2 후반) / 6 시야 새기·역전 실연[막3 onset] /
# 7 마지막 접근(탈출/진실 — index 7 맵은 보스/탈출/??? 라 시야붕괴 미적용, 마무리 톤. onset은 6에서).
# (A2에서 막·맵 늘면 행도 함께 확대.)
const BRIEFINGS_BY_BAND: Dictionary = {
	"cold": [
		...
		["서버실이 바로 앞입니다. 마지막 경비선입니다. 여기만 지나면 됩니다."],
	...
	"thaw": [
		...
		["서버실이 거의 코앞이에요. 여기가 마지막 길목이에요. 조금만 더요."],
