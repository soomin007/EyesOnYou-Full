...
static func _rooftops() -> Dictionary:
	return {
		...
		"platforms": [
			# 점프 등급(2026-06-24 재배치): S = 단순점프(Δ≤95), D = 더블점프 여유(Δ~130).
			# 빡빡한 Δ160(더블 192 대비 여유 32px)은 전면 제거. 슬랩 도착은 S로 "올라섰다" 리듬.
			# 지상(3050) → 저층 옥상(2730)
			{"pos": Vector2(560, 2955), "w": 220.0},  # Δ95 (S) 비상사다리
			{"pos": Vector2(740, 2860), "w": 220.0},  # Δ95 (S) 비상사다리
			{"pos": Vector2(640, 2730), "w": 480.0},  # Δ130 (D) — ROOF 1 (저층 옥상, patrol)

			# 저층 → 중층 옥상(2380)
			{"pos": Vector2(420, 2600), "w": 180.0},  # Δ130 (D) HVAC 박스
			{"pos": Vector2(700, 2510), "w": 180.0},  # Δ90 (S) AC 유니트
			{"pos": Vector2(560, 2380), "w": 440.0},  # Δ130 (D) — ROOF 2 (중층 옥상)

			# 중층 → 분기 옥상(2030)
			{"pos": Vector2(760, 2250), "w": 180.0},  # Δ130 (D) 스카이라이트
			{"pos": Vector2(520, 2160), "w": 180.0},  # Δ90 (S)
			{"pos": Vector2(640, 2030), "w": 520.0},  # Δ130 (D) — ROOF 3 (분기점, patrol)

			# 비밀 통로 — ROOF 3에서 우측 더블점프 → 안테나 → 비밀 옥상. XP 2 + HP 1 보너스.
			# (stage 0엔 글라이드 미보유라 글라이드 게이트 없음 — 비밀 옥상 보상으로 충분.)
			{"pos": Vector2(1130, 2110), "w": 100.0}, # 안테나 발판
			{"pos": Vector2(1180, 2210), "w": 140.0}, # 비밀 옥상 끝 — XP 2 + HP 1

			# 분기 우측(XP) — 노출된 옥상 가장자리. 짧음(2 발판).
			{"pos": Vector2(940, 1900), "w": 200.0},  # Δ130 (D)
			{"pos": Vector2(1060, 1810), "w": 180.0}, # Δ90 (S) — XP 끝

			# 분기 좌측(HP) — 안전한 안쪽. 짧음(2 발판).
			{"pos": Vector2(340, 1900), "w": 200.0},  # Δ130 (D)
			{"pos": Vector2(220, 1810), "w": 180.0},  # Δ90 (S) — HP 끝

			# 합류
			{"pos": Vector2(560, 1680), "w": 440.0},  # Δ130 (D) — ROOF 4 합류

			# 상층 — sniper post 2개 + 슬랩. 이전 Δ160 8연속(점프킹) → S/D 리듬으로 교체.
			{"pos": Vector2(720, 1550), "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(620, 1460), "w": 400.0},  # Δ90 (S) — ROOF 5 (sniper post 1)
			{"pos": Vector2(760, 1330), "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(540, 1200), "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(660, 1110), "w": 360.0},  # Δ90 (S) — 슬랩
			{"pos": Vector2(780, 980),  "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(560, 850),  "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(640, 760),  "w": 400.0},  # Δ90 (S) — ROOF 6 (sniper post 2, HP 보상)
			{"pos": Vector2(760, 630),  "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(540, 500),  "w": 240.0},  # Δ130 (D)
			{"pos": Vector2(640, 410),  "w": 360.0},  # Δ90 (S) — 슬랩
			{"pos": Vector2(560, 280),  "w": 240.0},  # Δ130 (D) — 골 직전 (→ goal 200, Δ80)

			# 저격 둥지 — 메인 경로 밖 측면 단독 발판(올라설 필요 없음, 회피 전용).
			{"pos": Vector2(1150, 1430), "w": 64.0},  # 둥지(중상층) — 우측 (ROOF 5 근처)
			{"pos": Vector2(130, 730),   "w": 64.0},  # 둥지(상층) — 좌측 (ROOF 6 근처)
		],
		...
		"enemies": {
			...
			"patrol": [Vector2(640, 2700.0), Vector2(640, 2000.0), Vector2(560, 1650.0)],
			"sniper": [
				Vector2(1150, 1402.0),  # 둥지(중상층, ROOF 5 근처)
				Vector2(130, 702.0),    # 둥지(상층, ROOF 6 근처)
			],
			"drone":  [],
			"bomber": [], "shield": [],
		},
		"rewards": {
			...
			"xp_orbs":    [
				Vector2(1040, 1780.0), Vector2(1080, 1780.0),   # 분기 우 XP 끝(1810) 위
				Vector2(1160, 2180.0), Vector2(1200, 2180.0),   # 비밀 옥상(2210) 위
				Vector2(620, 1430.0), Vector2(660, 1430.0),     # ROOF 5(1460) 보너스
			],
			# 글라이드 게이트 없음 — stage 0 맵이라 글라이드 미보유.
			"hp_pickups": [
				Vector2(220, 1780.0),    # 분기 좌 HP 끝(1810) 위
				Vector2(1180, 2180.0),   # 비밀 옥상
				Vector2(640, 730.0),     # ROOF 6(760) 보충
			],
		},
		"spikes": [],
	}
