...
# ─── 12. 도전 방 · 블랙아웃 런 (HORIZONTAL, 33s 시간 제한 · 피격 -5s) ──
# world_layout §3.2. Stage 4 분기 의도적 선택지.
# 강화: 좁은 발판(80~140px) + 가시 함정 + drone/bomber 압박 + 직선상 patrol 5.
# 피격 = 시간 -5s(1 hit fail 폐지 2026-08-29) · "긴장감"은 정밀 이동 + 시야 제한 + 시간에서 나옴.
...
static func _blackout() -> Dictionary:
	return {
		...
		# Stage가 인식해 블랙아웃 + 타이머 + 피격 시간 페널티 적용. 통로만큼 길어져 30 → 33s.
		"challenge":          true,
		"challenge_time":     33.0,
...
static func _server_stacks() -> Dictionary:
	return {
		...
		"no_spike_fallback": true,
		"route_lines": [
			{"x": 340.0, "who": "veil", "text": "서버 구역입니다. 서버 위 발판이 유리합니다. 위에서 보고, 위에서 쏘십시오.", "dur": 3.4},
		],
	}
