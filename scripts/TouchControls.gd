...
# 버튼 상태 흐림 계수(알파 배율). 미보유=더 흐리게(비활성), 쿨다운=중간(눌러도 무시되지만 곧 준비됨).
const DIM_UNOWNED: float = 0.22
const DIM_COOLDOWN: float = 0.40
...
# dash/skill 버튼의 미보유·쿨다운·전투차단(??? 방 등)을 매 프레임 반영한다.
# 상태(vis/en/dim)는 버튼 dict에 기록해 _button_at(입력)·_draw_button(그리기)이 공유한다.
func _update_states() -> void:
	var restrict: bool = GameState.restrict_combat_input
	var has_dash: bool = GameState.has_skill("dash")
	var skill_tier: int = GameState.get_skill_tier("explosive")
	var dash_cd: float = 0.0
	var skill_cd: float = 0.0
	var charges: int = 1
	var pl: Node = get_tree().get_first_node_in_group("player")
	if pl != null and is_instance_valid(pl):
		dash_cd = float(pl.get("dash_cd"))
		skill_cd = float(pl.get("skill_cd"))
		charges = int(pl.get("skill_charges"))
	var sig: String = ""
	for i in _buttons.size():
		var b: Dictionary = _buttons[i]
		var action: String = b["action"]
		...
		match action:
			...
			"skill":
				if restrict:
					...
					en = false
					dim = DIM_UNOWNED
				elif skill_cd > 0.0 or charges <= 0:
					dim = DIM_COOLDOWN
		b["vis"] = vis
		b["en"] = en
		b["dim"] = dim
		sig += "%d%d%.2f|" % [1 if vis else 0, 1 if en else 0, dim]
	if sig != _state_sig:
		_state_sig = sig
		if _pad != null:
			_pad.queue_redraw()
...
func _draw_button(pad: Control, b: Dictionary, down: bool) -> void:
	if not bool(b.get("vis", true)):
		return  # 전투 차단(??? 방 등)으로 숨긴 버튼
	var c: Vector2 = b["center"]
	var r: float = b["radius"]
	...
	var icon_c: Color = ICON_COL
	bg_c.a *= dim
	edge_c.a *= dim
	icon_c.a *= dim
	pad.draw_circle(c, r, bg_c)
	pad.draw_arc(c, r, 0.0, TAU, 40, edge_c, 2.5, true)
	if kind == "tri_left":
		pad.draw_colored_polygon(PackedVector2Array([
			c + Vector2(-r * 0.38, 0.0), c + Vector2(r * 0.26, -r * 0.42), c + Vector2(r * 0.26, r * 0.42)]), icon_c)
	elif kind == "tri_right":
		pad.draw_colored_polygon(PackedVector2Array([
			c + Vector2(r * 0.38, 0.0), c + Vector2(-r * 0.26, -r * 0.42), c + Vector2(-r * 0.26, r * 0.42)]), icon_c)
	elif kind == "tri_down":
		pad.draw_colored_polygon(PackedVector2Array([
			c + Vector2(0.0, r * 0.38), c + Vector2(-r * 0.42, -r * 0.26), c + Vector2(r * 0.42, -r * 0.26)]), icon_c)
	elif kind == "pause":
		var bw: float = r * 0.22
		var bh: float = r * 0.72
		pad.draw_rect(Rect2(c + Vector2(-bw * 1.6, -bh * 0.5), Vector2(bw, bh)), icon_c)
		pad.draw_rect(Rect2(c + Vector2(bw * 0.6, -bh * 0.5), Vector2(bw, bh)), icon_c)
	elif kind == "text" and _font != null:
		var label: String = b["label"]
		var fs: int = 28
		pad.draw_string(_font, c + Vector2(-r, fs * 0.34), label, HORIZONTAL_ALIGNMENT_CENTER, r * 2.0, fs, icon_c)
