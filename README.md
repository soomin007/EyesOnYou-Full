...
> **"AI 파트너 VEIL과 함께 시설 하나를 관통하는 횡스크롤 다회차 서사 액션. 마지막에 VEIL이 누구였는지가 드러난다."**

### ▶ [브라우저에서 바로 플레이](https://soomin007.github.io/EyesOnYou/)

> GitHub Pages 자동 배포 (main 푸시 시 갱신). 데스크톱 Chrome/Firefox 권장. 첫 로딩 ~10s.

근미래 민간 보안기업의 현장 요원이 되어, 상황실 AI 파트너 **VEIL**의 조언을 들으며(혹은 무시하며) 5막 15개의 짧은 횡스크롤 스테이지를 통과한다. 누적된 선택(VEIL 추천 수용률 × 전투/우회 비율)과 마지막에 직접 고르는 처리 방식이 9종 엔딩 중 하나로 이어지고, 그 과정에서 VEIL의 말투가 차갑게→따뜻하게 변한다.

- **엔진**: Godot 4.6 (GL Compatibility, physics interpolation 활성)
- **장르**: 횡스크롤 액션 어드벤처 + 다회차 서사(런 구조 차용 — 로그라이트 아님: 절차 생성·
  사망 기반 메타 성장 없음. 루트/빌드 드래프트와 다회차 변주가 런 구조의 몫)
- **구성**: 5막 15스테이지 · 맵 36종 · 엔딩 9종
- **플랫폼**: 브라우저(웹, 위 링크) · 데스크톱 · 모바일 터치 · 키보드 / 게임패드
...
**일반 모드**: HP 3, 5막 15 스테이지, 막3 보스 3페이즈 + 막5 최종 보스 2막 구성, 모든 적 등장.
**스토리 모드**: HP 무제한, 5 스테이지, 보스 단순화, 드론 없음. 키보드/패드가 익숙하지 않은 사람을 위한 짧은 코스.

**5막 구성** (`GameState.ACTS`): 1 침투 → 2 잠입 → 3 핵심부(보스 SENTINEL) → 4 추적 → 5 대면.
막마다 맵 성격·적 팔레트·난이도 램프·BGM이 갈리고, 막 진입 시 "ACT N · 이름" 카드가 뜬다.

## 엔딩 분기 (9종)

마지막 회수 지점에서 **드라이브를 어떻게 처리할지 직접 고른다**(반출 / 파기 / 은닉 / 잔류).
그 선택이 곧바로 **탈출 스테이지 4종**으로 갈라지고, 엔딩도 처리 × 신뢰로 정해진다.

| 축 | 값 | 영향 |
|---|---|---|
| **처리** (플레이어 선택) | 반출 · 파기 · 은닉 · 잔류 | 엔딩 갈래 + 마지막 스테이지의 성격이 통째로 바뀜 |
| **신뢰** (hi / lo) | 추천 수용률 ≥ 50% 여부(`followed_count*2 ≥ rec_count`) | 같은 처리라도 VEIL과의 관계가 다른 결말 |
| **진실** (truth) | ??? 방에서 격리된 VEIL들을 만났는가 | 다 알고 한 선택. 처리·신뢰를 덮고 진실 엔딩으로 수렴 |

→ 처리 4 × 신뢰 2 + 진실 1 = **9종**(`scripts/EndingResolver.gd`). 엔딩마다 전용 에필로그가 붙는다.

- 별도로 **어투 trust**(0에서 climbing)가 VEIL 말투를 차갑게→따뜻하게 바꾼다 — 엔딩과는 분리(아래 "VEIL 어투 아크").
- 전투·도전 태그 선택 누적(`aggression_score`)은 데모 시절 엔딩 축이었고, 지금은 성향 추적으로 남아 있다.
...
|---|---|---|
| 좌우 이동 | A / D, ←/→ | 좌스틱 / D-Pad |
| 점프 (이중 점프) | W / Space / Z / ↑ | A |
| 아래 내려가기 | S / ↓ | ↓ (좌스틱/D-Pad) |
| 사격 | J / X *(마우스 좌는 설정에서 바인드)* | X 또는 RT |
| 대시 | K / C / Shift | B 또는 RB |
| 액티브 스킬 | L / V *(마우스 우는 설정에서 바인드)* | Y |
| 일시정지 | ESC / P | START |
| 전체화면 토글 | F11 | |
| UI 확정 | Enter / Space | A |
| UI 취소 / 뒤로 | ESC | B |
...
**모바일**: 터치 패드(멀티터치)로 전부 플레이 가능. 보유하지 않은 스킬·쿨다운 중인 버튼은 흐려지고,
세로로 돌리면 자동 일시정지된다.

---

## 맵 36종 + 진행 흐름 (Dead Cells 스타일)

각 맵마다 등장 가능 스테이지가 다름. Dead Cells 스타일로 매 스테이지마다 풀에서 2~3개를 추첨해 보여준다.

> 아래 표는 **데모 12맵**(대체로 막1~3 구간)이다. `min~max stage` 값은 5막 재구조화(15스테이지)로
> 재분배됐으므로 **`scripts/RouteData.gd::ALL_ROUTES`가 진실**이고, 표는 맵 성격 파악용으로 본다.
...
- **일반 맵**: 서버 회랑 · 주차장 · 변전소 · 실험구역 · 철거구역 · 펌프장 · 중계소 · 창고 · 검문소 ·
  통제실 복도 · 응축기 · 외곽 경계 · 시련의 길. 맵마다 코드로 그린 전용 배경(시그니처 소품)이 붙는다.
- **기믹 맵**(적게, 제대로): 화물 리프트(이동 발판) · 차량 엄폐 통로(부서지는 엄폐 + 저격 레이스) ·
  붕괴 갱도(뒤에서 밀려오는 벽 = 강제 전진) · 반응로 제어실(코어 방어) · 감시 회랑(스캔 빔 리듬) ·
  저지선(엄폐 농성).
- **막5 전용**: 회수(`route_core_recovery`, 최종 보스 1막) + **처리별 탈출 4종**
  (반출=봉쇄 게이트 총력 저지 / 파기=붕괴 회랑 탈출 / 은닉=정비 갱도 잠입 / 잔류=무인 회랑).
...
| 호출병 (Caller) | 직접 공격 없음 · 다가가면 물러나고, 안테나 신호 후 증원 호출(불려 온 증원은 처치 보상 없음) | 신호 링이 보이는 동안 끊거나 최우선 처치 (HP 2) |
| 교란기 (Jammer) | 고정 방출 장치 — 바이올렛 반경 안 VEIL 마커 소등 | 본체를 직접 보고 부수면 시야 복구 (막3+) |
| 강화 개체 (Elite) | 막4부터 등장하는 라이벌의 정예 · 보랏빛 계급장, 타입별 템포 강화(HP↑·2연 버스트/투하·방패병은 폭발 면역) | 상대법은 기반 타입과 같음. 계급장 보이면 우선순위 판단 (상세=`docs/archive/elite_enemies_plan.md`) |
...
| 계열 | 라인 | T1 → T2 → T3 |
|---|---|---|
| 전투 | 사격 강화 (fire_boost) | 데미지 +1 → 속사(연사 +40%·사격 중 감속 완화) → 1체 관통 |
| 전투 | 다중사격 (multishot) | 삼연사(부채꼴 3) → 오연사(5) → 5발 + 사거리 연장 |
| 전투 | 폭발물 (explosive, 액티브) | 광역 처치(3s) → 반경 +30%·쿨 2.5s → 2회 충전 |
| 이동 | 공중 활강 (glide) | 홀드 활강(낙하 중 점프 키 홀드로 천천히 낙하) → 삼단 점프(공중 점프 +1) → 유도 사격(공중 사격이 적을 강하게 유도·데미지 +1) |
...
| **VEIL 시야** (VeilSight) | VEIL이 화면 안팎 위협을 마킹·말로 짚어줌. 마지막 막에선 마킹이 흐려지고 일부는 영영 안 보임(시야 역전) + 진입 시 함정/매복 미리 경고. |
...
| **이동 발판** (MovingPlatform) | 핑퐁 왕복 발판, 올라탄 것을 자동으로 실어 나름. 스파이크 구덩이 위를 페리로 건너는 화물 리프트. |
| **부서지는 엄폐** (DestructibleCover) | 적탄에 맞을수록 금이 가다 파괴되는 솔리드 엄폐물. 숨으면 안전하지만 **엄폐가 유한 자원**이라 머물수록 노출된다. |
| **추격 벽** (ChaseHazard) | 뒤에서 밀려오는 붕괴 벽. 삼켜지면 치명. 멈추면 죽는 강제 전진 구간. |
| **방어 코어** (DefenseCore) | 지켜야 할 코어 + 침입 판정 돔. 적이 돔에 들어와 있는 동안 코어 HP가 깎여, 자리를 비울 수 없는 고정 방어가 된다. |
| **스캔 빔** (SweepBeam) | 통로를 주기적으로 훑는 수직 빔(쉼→경고→훑기). 빔보다 빠를 수 없어 차폐 니치로 대피했다 달리는 stop-and-go 리듬. |
| **위장 함정** (DisguisedSpike) | 안전한 바닥처럼 보이는 진짜 가시. 항상 붉은 지직거림 tell이 켜져 있고, 가까이 가거나 밟는 순간 정체가 드러난다(후반 시설의 "거짓 렌더" 계열). |
| **재밍 필드** (Jammer) | 교란기 반경 안에서는 VEIL 마커가 꺼진 **날것 시야**가 된다. 본체를 눈으로 찾아 부수면 복구. |
...
├── icon.svg
├── docs/
│   ├── INDEX.md                문서 지도: "이 주제는 이 문서가 진실" (여기부터 보면 된다)
│   ├── SPEC.md                 구현 사양 (씬 구조·시스템 도식·영속화 스키마)
│   ├── STORY.md                스토리 캐논 + 게임 텍스트 (단일 진실)
│   ├── design/
│   │   ├── backlog.md          미착수 작업 단일 소스 (다음 작업 후보 + ★추천)
│   │   ├── known_issues.md     반복 방지 — 버그/설계 함정 "증상→원인→방지책"
│   │   ├── act_identity.md     막별 정체성 5요소(성격·팔레트·램프·전환비트·대비좌표)
│   │   ├── rival_veil_concept.md  후반 라이벌 VEIL 설계 (거짓 렌더·최종 보스 2막 구성)
│   │   ├── replay_support_plan.md 다회차 3축 (기록 재진입·처리별 탈출·라이벌 기억)
│   │   ├── lore_expansion.md   관측 프레임 세계관 보강
│   │   ├── dialogue_review.md  대사 전수 검토 (수정안 기입식)
│   │   ├── growth_system.md    스킬 트리 3계열×3티어 + XP·상성 설계
│   │   ├── world_layout.md     맵 템플릿 + 맵 좌표 + 보스/이스터에그 명세
│   │   └── show_dont_tell.md   "글로 명시 < 체험으로 체득" 톤 원칙 + 적용 후보
│   └── archive/                역할 끝난 설계·분석 문서 (이력 보존, 참조 금지)
├── assets/
...
│   ├── stage / death / ending / credits
│   ├── core_tunnel             최종 막 코어 터널 (유사 1인칭 원근 렌더러)
│   └── settings                키바인드 + 디버그(연습장·메타 관리) 탭
├── scripts/
│   ├── GameState.gd            AutoLoad — 진행도/점수/스킬/루트/도감 영속 + 막 스냅샷
│   ├── SceneRouter.gd
│   ├── RouteData.gd            루트 풀 36종 (id/risk/reward/tags/min~max stage)
│   ├── MapData.gd              맵 좌표 + 함정/트립와이어 + 보스/웨이브/기믹/이스터에그 메타
...
│   ├── MovingPlatform.gd / DestructibleCover.gd / ChaseHazard.gd   기믹 프리미티브 (발판·엄폐·추격 벽)
│   ├── DefenseCore.gd / SweepBeam.gd / DisguisedSpike.gd           기믹 프리미티브 (코어 방어·스캔 빔·위장 함정)
│   ├── Grenade.gd              액티브 스킬: 포물선 수류탄 (차징 사거리 + 궤도 미리보기)
│   ├── CoreTunnel.gd / FalseVeil.gd     최종 막: 코어 터널 · 거짓 VEIL 분신전
│   ├── DisposalChoiceOverlay.gd / ReentryOverlay.gd   처리 선택 4지선다 · 기록 재진입 막 노드
│   ├── TouchControls.gd / OrientationGuard.gd   모바일 터치 패드 · 화면 방향 유도/자동 일시정지
│   ├── VeilSight.gd            VEIL 위협 마킹 + 시야 비네트 + 후반 시야 붕괴
...
- **기록 재진입(팔림프세스트)**: 한 번 완주하면 막 경계 스냅샷이 `user://palimpsest.cfg`에 남아, 타이틀에서 **막 단위로 되돌아가 다시 쓰기**가 열린다. 처음부터 다시 하지 않고 후반 선택만 바꿔 다른 엔딩을 볼 수 있다. 저장 구조는 이어하기와 같은 직렬화를 공유한다.
- **다회차를 기억하는 쪽은 따로 있다**: 완주·처리 이력·간파 여부가 설정 파일에 누적돼, 반복 플레이에서 시설 쪽 반응이 달라진다. (자세히 쓰면 스포일러라 여기까지.)
...
핵심 시스템은 완성 단계: 5막 15스테이지 · 맵 36종 분기 · 엔딩 9종(처리 선택 + 처리별 탈출 4종),
3계열 8라인×3티어 스킬 트리, 적 6종 + 엘리트 + 보스 2종, 기믹 프리미티브 7종,
VEIL 어투 아크(신뢰 구동), 기록 재진입, UI 시각화(텍스트→그래픽), 음악·효과음(전부 AI 생성),
자동저장/이어하기·다회차 변형, 모바일 터치, Xbox 컨트롤러, 웹 자동 배포.

**남은 것은 대체로 실플레이 튜닝과 대사 확정**(막4/5 밸런스, 14장 보스 손맛, 미확정 문구)이다.
