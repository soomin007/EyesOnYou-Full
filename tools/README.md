...
| RuleSmoke | `godot --headless --path . --audio-driver Dummy tools/rule_smoke.tscn` | 실플레이 규칙 3종 실인스턴스 단언: 저격 조준 고정(비키면 빗나감·제자리 명중·사거리 600·거치대 하향 사각) · 도전방(피격 -5s·비상등·시간 초과 = 조명 복구+방 계속+프리미엄 없음) · 방류구(판정 = 플랜지 입구부터·하우징 z) · 이동 발판 탑승(수평 흔들림 0 · 수직 접지 유지) |
